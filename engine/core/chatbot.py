...
def get_niva_response(query):
    ...
    full_prompt = f"{NIVA_SYSTEM_PROMPT}\nHistory:\n{history_context}\n\nUser: {query}\nNiva:"
    ...
    fallback_msg = {"type": "message", "content": f"Sir, my neural link is currently fluctuating. I captured your intent: '{query}'."}
...
def _check_local_action(query):
    ...
    if 'shutdown' in q or 'terminate' in q:
        return {"type": "action", "action_name": "system_command", "parameters": {"command": "shutdown"}, "confirmation_required": True}
    if 'chrome' in q or 'browser' in q:
        return {"type": "action", "action_name": "system_command", "parameters": {"command": "chrome"}, "confirmation_required": False}
    if 'calculate' in q or 'math' in q:
        return {"type": "action", "action_name": "search", "parameters": {"target": "math_engine", "query": q}, "confirmation_required": False}
    return None
